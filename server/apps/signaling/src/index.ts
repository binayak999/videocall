...
function rosterFromSockets(
  ...
  return sockets.map((s) => {
    ...
    return { peerId: s.id, userId: d.userId, userName: d.userName };
  });
}
...
io.use((socket, next) => {
  const raw = socket.handshake.auth as { token?: unknown; cameraToken?: unknown };

  // Camera-source auth: short-lived token generated by the host
...
io.on("connection", (socket) => {
  ...
  /** Host generates a one-time token for a remote camera source to connect. */
  socket.on("camera:generate-token", (msg: unknown, cb: unknown) => {
    ...
    const hostUserId = roomHostUserId.get(room);
    if (!hostUserId || hostUserId !== sd.userId) { ack({ ok: false, error: "Not host" }); return; }
    const label =
  ...
  /** Camera source announces it is ready — signals the host to start WebRTC. */
  ...
  /** Host asks a camera source to send an offer. */
  ...
  /** Camera source sends WebRTC offer to host. */
  ...
  /** Host sends WebRTC answer back to a camera source. */
  ...
  /** ICE candidates between camera source and host (bidirectional). */
  ...
  socket.on("meeting:join", async (code: unknown, cb: unknown) => {
    ...
    let peerRosterForAck: { peerId: string; userId: string; userName: string }[] = [];
    try {
  ...
  socket.on("meeting:join-decision", async (msg: unknown) => {
    ...
    const reqData = requesterSocket.data as MeetingSocketData;
    requesterSocket.to(pending.room).emit("meeting:peer-joined", {
      peerId: requesterSocket.id,
