...
export function MeetingPage() {
  ...
  function applyHostModeFromJoinAck(a: Record<string, unknown>) {
    if (a.isHost === true) return
    if (a.hostMode !== 'mesh' && a.hostMode !== 'livekit') return
    const hostMode = a.hostMode as RtcMode
    if (hostMode === resolvedRtcMode()) return
    writeRtcModeToStorage(hostMode)
    mediaModeRef.current = hostMode
    showToast(`Host is using ${hostMode} mode — switching automatically`, 4000)
  }
  ...
  async function connect() {
    ...
    socket.emit('meeting:join', { code, rtcMode: resolvedRtcMode() }, (ack: unknown) => {
